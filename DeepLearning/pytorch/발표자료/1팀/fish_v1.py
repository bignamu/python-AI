...
# # 3. 히트맵으로 수치확인
# heatmap_data = fish_file[['Weight', 'Length1', 'Length2', 'Length3', 'Height', 'Width']]
#
# colormap = plt.cm.PuBu
# plt.title("Correlation of Features", y=1.05, size=15)
# sns.heatmap(heatmap_data, linewidths=0.1, vmax=1.0, square=True, fmt='f',
#             cmap=colormap, linecolor="white", annot=True, annot_kws={"size": 16})
# plt.show()
# # 4. Perch / Roach / Whitefish 의 분포를 확인
...
# 모듈에 가져온 데이터 넣기
# dataset & loader 설정
batch_size = 3                      # 1 -> 2 -> 3 -> 6 / 갈수록 loss 값이 작아짐(학습시키는 양이 증가되는거라 당연한 것)
train_dataset = CustomDataset(x_train, y_train, transforms=None)
train_loader = DataLoader(train_dataset, batch_size=batch_size,         # BatchNorm1d을 활성화 시키면
                          shuffle=True, drop_last=True)                 # batch_size > 1
# device & model 설정
...
optimizer = optim.Adam(model.parameters(), lr=0.03)       # 결과의 값들이 다들 비슷하다.
...
for i in range(epoch):
    ...
    if (total_loss/len(x_train)) < min_loss:                  # 가장 loss 값이 작을 때 저장
        min_loss = total_loss
...
for x, y in test_loader:
    ...
    # 정확도 계산 / 2, 6을 정답으로 결과가 계속 나온다. 데이터의 양의 차이 때문인가?
